...
float dtw_dist(const float *x, const float *y, int32_t nx, int32_t ny) {
    ...
    for (i = 1; i <= nx; i++) {
        k = (2 - i) % ncol;
        if (k < 0) k += ncol;
        if (k == 0) prev_val = cost_mat[ncol - 1];
        else prev_val = cost_mat[k - 1];
        for (j = 1; j <= ny; j++) {
            cost = pow(x[i - 1] - y[j - 1], 2);
            if (k == ncol - 1) next_val = cost_mat[0];
            else next_val = cost_mat[k + 1];
            cost_mat[k] = cost + min3(next_val,         // insertion
                                      prev_val,         // deletion
                                      cost_mat[k]);     // match
            prev_val = cost_mat[k++];
            if (k == ncol) k = 0;
